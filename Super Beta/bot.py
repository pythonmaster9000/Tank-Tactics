...
async def start(interaction):
    ...
    if str(interaction.guild.id) in game_data:
        if '0' in game_data[str(interaction.guild.id)]:
            await interaction.response.send_message("there is already a game, if this is an error try !testdelete", ephemeral=True)
            return
...
async def kickplayer(ctx, player):
    ...
    try:
        for message in view_data[str(ctx.guild.id)]:
            if view_data[str(ctx.guild.id)][message]['player_id'] == str(ctx.message.author.id) and view_data[str(ctx.guild.id)][message]['channel_id'] == str(ctx.channel.id):
