...
async def on_ready():
    ...
    for server in view_data:
        for message_id in view_data[server]:
            if not game_data[server]['0']['config']['started']:
                try:
                    client.add_view((views.RecruitmentController(client, server)))
                except KeyError:
                    print('key error adding recruitment')
                    pass
                continue
...
# @client.tree.command()
# @app_commands.describe(fruits='The fruit to choose')
# @app_commands.choices(fruits=[
#            app_commands.Choice(name='apple2', value=1),
#            app_commands.Choice(name='banana3', value=2),
#            app_commands.Choice(name='cherry4', value=3),
#        ])
...
async def start(interaction):
    if interaction.user.id != 193878633654386688:
        await interaction.response.send_message("you can't use that command pal", ephemeral=True)
        return
    with open('gamedata.json') as file:
        game_data = json.load(file)
    if str(interaction.guild.id) in game_data:
        if '0' in game_data[str(interaction.guild.id)]:
            await interaction.response.send_message("there is already a game, if this is an error try !testdelete",
                                                    ephemeral=True)
            return
    view = views.RecruitmentController(client, str(interaction.guild.id))
    emb = discord.Embed(title='Press to join', description='players:', color=44469)
...
# @client.tree.command()
# async def kickplayer(interaction, player):
#    ...

@client.command()
async def kickplayer(ctx, player):
    try:
        player_id = int(player.replace('<', '').replace('@', '').replace('>', ''))
    except ValueError:
        return
    if player_id == 1103215651407020032:
        return
    with open('viewdata.json') as file:
        view_data = json.load(file)
    try:
        for message in view_data[str(ctx.guild.id)]:
            if view_data[str(ctx.guild.id)][message]['player_id'] == str(ctx.message.author.id) and \
                    view_data[str(ctx.guild.id)][message]['channel_id'] == str(ctx.channel.id):
                guy = await client.fetch_user(player_id)
                await ctx.channel.set_permissions(target=guy, overwrite=discord.PermissionOverwrite(view_channel=False))
    except:
        return
...
#        view_data = json.load(file)
#    try:
#        # go through all channels and delete them
#        del view_data[str(ctx.guild.id)]
#    except KeyError:
#        pass
#    with open('viewdata.json', 'w') as outfile:
#        json.dump(view_data, outfile, indent=4)
...
#        game_data = json.load(file)
#    try:
#        del game_data[str(ctx.guild.id)]
#    except KeyError:
#        pass
#    with open('gamedata.json', 'w') as outfile:
#        json.dump(game_data, outfile, indent=4)
...
client.run('MTEwMzIxNTY1MTQwNzAyMDAzMg.G9gCxP.QkzhPM1rqOkeM25X3mhdFPPVx5l8GGFKv4sLsk')
