...
class DrawMap:
    def __init__(self, server, game_id, player_id=None):
        with open(r'gamedata.json') as file:
    ...
    def drawfooter(self):
        ...
        background = Image.open(r'footer.png')
        background.convert('RGBA')
        #print(background.info)
        font = ImageFont.truetype(r'Flome Regular.otf', size=95,
                                  encoding='utf-8')
        namefont = ImageFont.truetype(r'Flome Regular.otf', size=75,
                                      encoding='utf-8')


        tank = Image.open(fr'pfpcache\{player}.jpg').convert('RGBA')
        ...
        draw.text((0, 185), self.game_info["players"][player]["name"], font=namefont, fill=(57, 62, 70), stroke_width=1,
                  stroke_fill=(238, 238, 238))
        heart = Image.open(r'bigheart.png').convert('RGBA')
    ...
    def drawplayermap(self, spectate=False, win=False):
        background = Image.open(r'background3.jpg')
        background = background.convert('RGBA')
        # font = ImageFont.truetype('arial.ttf', size=45)
        font = ImageFont.truetype(r'Flome Regular.otf', size=40,
                                  encoding='utf-8')
        namefont = ImageFont.truetype(r'Flome Regular.otf', size=30,
                                      encoding='utf-8')

        victoryfont = ImageFont.truetype(r'Flome Regular.otf', size=90,
                                      encoding='utf-8')
        for player in self.game_info["players"]:
            ...
            if win:
                ...
                winav = Image.open(fr"pfpcache\{list(self.game_info['players'].keys())[0]}.jpg")
