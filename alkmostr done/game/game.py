...
def inbounds(xy):
    return xy[0] in range(0, 1601) and xy[1] in range(0, 801)
...
class GameClient:
    ...
    def attack(self, enemy_coords):
        if self.check_ap() <= 0:
            return False
        if abs(enemy_coords[0] - self.get_player_coords()[0]) <= 2 * 100 and abs(
                enemy_coords[1] - self.get_player_coords()[1] <= 2 * 100):
