...
class RecruitmentController(discord.ui.View):
    ...
    async def start(self, interaction: discord.interactions.Interaction, button):
        print('starting game')
        await interaction.message.edit(content="Spectate the battlefield", attachments=[drawing.DrawMap(str(interaction.guild.id), '0').drawplayermap(spectate=True)], view=None)
